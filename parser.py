    ...
    for page in range(1, max_pages + 1):
        ...
        if response.status_code == 200:
            ...
            for element in recipe_elements:
                title_element = element.find('a', class_='listRecipieTitle')  # название и ссылка на рецепт
                if title_element:
                ...
                # Изображение рецепта
                image_element = element.find('span', class_='thumb').find('img')
                image_link = image_element['src'] if image_element else "Изображение не найдено"

                # Время приготовления
                cook_time_element = element.find('div', class_='cook-time').find('span', class_='value')
                cook_time = cook_time_element.text.strip() if cook_time_element else "Не указано"
                
                # Автор рецепта
                owner_element = element.find('div', class_='owner').find('span')
            ...
            # Если запрос не успешен, выводим сообщение об ошибке и завершаем
            print(f"Ошибка при запросе страницы {page}: {response.status_code}")
            return [{"Ошибка": f"Ошибка при запросе: {response.status_code}"}]
...
def get_recipes_povar(URL):
    ...
    if response.status_code == 200:
        ...
        # Название рецепта
        recipe_name = povar_area.find("h1", class_="detailed").text.strip() if povar_area.find("h1", class_="detailed") else "Название не найдено"
...
# keyword = "рис"
# recipes = search_recipes(keyword, max_pages=10)


# # Вывод результата поиска
# for recipe in recipes:
#     print(f"Название: {recipe['title']}")
#     print(f"Ссылка на рецепт: {recipe['link']}")
#     print(f"Ссылка на фото: {recipe['image_link']}")
#     print(f"Время приготовления: {recipe['cook_time']}")
#     print(f"Автор: {recipe['author']}")
#     print("-" * 40)

# a = get_recipes_povar("https://povar.ru/recipes/ris_dlya_rollov_v_multivarke-10171.html")
# print(a)


# # Запуск функции и вывод всех советов
# all_advice = show_advice()
...
# advice = get_advice("https://povar.ru/art/mojno_li_zamorajivat_krasnuiu_ikru.html")

# if advice:
#     first_advice = advice[0]
#     print("Заголовок:", first_advice['headline'])
#     print("Изображение:", first_advice['article_img'])
#     print("Содержимое статьи:\n", first_advice['content'])
# else:
#     print("Совет не найден.")
